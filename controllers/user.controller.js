...
dotenv.config();
  ...
  async update(req, res) {
    try {
      ...
      // Basic validate
      if (!username||!email||!password||!status || !id) {
        return res.status(400).json({ error: 'Required fields are missing' });
      }
      // Verify if the User already exists
      const updateUserModel = await UserModel.update(id,{username,email,password,status});
      if (!updateUserModel) {
  ...
  try {
    ...
    if (existingUser) {
      ...
      if (!passwordHash) {
        ...
        const token = jwt.sign({id: existingUser.id, email: existingUser.email, status: existingUser.statusId}, process.env.JWT_SECRET, {
          expiresIn: '1h',
          algorithm: 'HS256'
        });
